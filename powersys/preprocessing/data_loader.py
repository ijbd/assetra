...
class DataLoader:
	...
	def _load_merra_temperature(self):
		...
		try:
			...
			logger.error(f'Expected merra power generation file located at: {power_generation_file}')
			raise e
	...
	def _load_eia_860_wind(self):
		...
		# find file
		eia_860_wind_file = Path(self.data_directory, 'eia860', f'3_2_Wind_Y{self.year}.xlsx')
		...
		try:
			...
			# get latitude array
			self.wind_latitude = self._plant_latitudes[eia_860_wind_df['Plant Code']].values
			self.wind_longitude = self._plant_longitudes[eia_860_wind_df['Plant Code']].values
	...
	def _load_eia_860_solar(self):
		...
		# find file
		eia_860_solar_file = Path(self.data_directory, 'eia860', f'3_3_Solar_Y{self.year}.xlsx')
		...
		try:
			...
			# get latitude array
			self.solar_latitude = self._plant_latitudes[eia_860_solar_df['Plant Code']].values
			self.solar_longitude = self._plant_longitudes[eia_860_solar_df['Plant Code']].values

		except FileNotFoundError as e:
			logger.error(f'Expected EIA-860 solar file located at: {eia_860_solar_file}')
			raise e
	...
	def _load_eia_860_storage(self):
		...
		# find file
		eia_860_storage_file = Path(self.data_directory, 'eia860', f'3_4_Energy_Storage_Y{self.year}.xlsx')
		...
		try:
			...
			self.storage_energy_capacity = eia_860_storage_df['Nameplate Energy Capacity (MWh)']

			# get latitude array
			self.storage_latitude = self._plant_latitudes[eia_860_storage_df['Plant Code']].values
			self.storage_longitude = self._plant_longitudes[eia_860_storage_df['Plant Code']].values

		except FileNotFoundError as e:
			logger.error(f'Expected EIA-860 storage file located at: {eia_860_storage_file}')
			raise e
