...
#. Pull the latest branch locally, so your contributions start from up-to-date code::
...
#. Release the new code:
    ...
    a. Pull the latest :code:`main` locally, so the version bump is applied on top of the merged changes::
 
        git checkout main
        git pull
 
    #. Bump the code version in :code:`assetra/__init__.py` and :code:`pyproject.toml`. Commit and push these changes.
...
#. Publish a release on GitHub:
    ...
    a. Go to :code:`Code > Tags > New Release`.
 
    #. Create a tag matching the new version number.
 
    #. Publish the release, using the auto-generate option to document the included changes.